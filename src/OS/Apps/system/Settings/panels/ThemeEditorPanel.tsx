...
export function ThemeEditorPanel({ onBack }: ThemeEditorPanelProps) {
  ...
  const customThemes = useSettingsStore(useShallow((state) => state.customThemes));
  const currentEra = useSettingsStore((state) => state.settings.appearance.era);
  const currentDarkMode = useSettingsStore((state) => state.settings.appearance.darkMode);
  const setSetting = useSettingsStore((state) => state.setSetting);

  // Derive current theme ID from era + darkMode for lookup in flat BUILT_IN_THEMES map
  const currentThemeId = currentDarkMode ? `${currentEra}-dark` : currentEra;
  ...
  const handleSave = useCallback(() => {
    ...
    // Custom themes are stored separately — no themeId to set in new model
    setHasChanges(false);
