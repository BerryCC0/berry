...
import { formatWei, truncateAddress } from '@/shared/format';
...
import { useAuctionById, type Bid } from '@/OS/Apps/nouns/Auction/hooks/useAuctionData';
import { getMinimumNextBid, formatBidAmount, isNounderNoun, NOUNDERS_ADDRESS, NOUNDERS_ENS } from '@/OS/Apps/nouns/Auction/utils/auctionHelpers';
import { extractPaletteIndices } from '../utils/paletteColors';
...
function getNounColors(noun: { background: number; body: number; head: number; glasses: number; accessory: number }): string[] {
  ...
  for (const part of parts) {
    const indices = extractPaletteIndices(part);
...
function BidRow({ bid }: { bid: Bid }) {
  const ensName = useEnsName(bid.bidder.id);
...
export function NounDetail({ nounId, onBack, onGoBack, onGoForward, canGoBack, canGoForward, onFilterByTrait }: NounDetailProps) {
  ...
  const ownerEns = useEnsName(owner ?? null);
  // Live ENS fallback for winner if DB hasn't been backfilled yet
  const winnerAddress = noun?.winner_address ?? null;
  const winnerEnsLive = useEnsName(!noun?.winner_ens && isValidAddress(winnerAddress) ? winnerAddress : null);
            ...
            {isNounder ? (
              <>
                <div className={styles.infoRow}>
                  <span className={styles.nounderBadge}>NOUNDER NOUN</span>
                </div>
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>AWARDED TO: </span>
                  <a href={etherscanUrl(NOUNDERS_ADDRESS)} target="_blank" rel="noopener noreferrer" className={styles.infoLink}>
                    {NOUNDERS_ENS}
                  </a>
                </div>
