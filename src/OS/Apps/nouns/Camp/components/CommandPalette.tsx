  ...
  const searchItems = useMemo<CommandItem[]>(() => {
    ...
    if (isQueryEns && resolvedAddress) {
      results.push({
        ...
        label: trimmedQuery,
        section: 'Voters',
        meta: truncateAddress(resolvedAddress),
    ...
    if (isQueryAddress && normalizedQuery.length === 42) {
      results.push({
        ...
        label: truncateAddress(normalizedQuery),
        section: 'Voters',
        meta: 'Go to voter',
    ...
    if (searchResults) {
      ...
      searchResults.voters.forEach(v => {
        ...
        results.push({
          id: `voter-${v.address}`,
          label: v.ensName || truncateAddress(v.address),
          section: 'Voters',
          meta: v.ensName ? truncateAddress(v.address) : `${v.delegatedVotes} votes`,
      ...
      searchResults.candidates.forEach(c => {
        results.push({
          ...
          section: 'Candidates',
          meta: truncateAddress(c.proposer),
