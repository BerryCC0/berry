/**
 * octant-vault-create-morpho — deploy a yield-donating Morpho Dragon vault.
 * Hardcoded asset: USDC (6 decimals). See vault-create-lido.ts for the
 * shared bundle pattern.
 */
...
const FACTORY = OCTANT_MORPHO_FACTORY_ADDRESS as Address;
const IMPL_FALLBACK = OCTANT_YIELD_DONATING_STRATEGY_ADDRESS as Address;
const ASSET = EXTERNAL_CONTRACTS.USDC.address;
const ASSET_DECIMALS = 6;

export const octantVaultCreateMorpho: TransactionActionDef<BaseVaultCreateFields> = {
  id: 'octant-vault-create-morpho',
  category: 'octant',
  name: 'Create Octant Morpho Vault',
  description:
    'Deploy a yield-donating Octant Dragon vault on Morpho (USDC underlying)',
  isMultiAction: true,
  fields: [
    ...
    { name: 'seedAmount', label: 'Initial Seed (USDC)', type: 'amount' },
  ...
  encode(values) {
    ...
    return assembleBundle('octant-vault-create-morpho', values, create, splitter, seed);
  },
