...
/**
 * Raw SQL row for a candidate list item (snake_case)
 */
interface ApiCandidateListRow {
  id: string;
  slug: string;
  proposer: string;
  title: string | null;
...
/**
 * Raw SQL row for candidate detail with action arrays and signatures
 */
interface ApiCandidateDetailRow extends ApiCandidateListRow {
  targets: string[] | null;
  values: string[] | null;
...
/**
 * Raw SQL row for a candidate signature
 */
interface ApiSignatureRow {
  id: string;
  signer: string;
  sig: string;
...
/**
 * Raw SQL row for candidate feedback
 */
interface ApiFeedbackRow {
  id: string;
  msg_sender: string;
  support: number;
  votes: string | null;
 ...
 */
function mapCandidateListItem(c: ApiCandidateListRow): Candidate {
  return {
    id: c.id,
    proposer: c.proposer,
    slug: c.slug,
 ...
 */
function mapCandidateDetail(c: ApiCandidateDetailRow): Candidate {
  ...
  const feedback: CandidateFeedback[] = (c.feedback || []).map((f: ApiFeedbackRow) => ({
    id: f.id,
    voter: f.msg_sender,
    support: f.support,
