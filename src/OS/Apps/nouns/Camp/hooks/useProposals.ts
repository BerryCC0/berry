...
/**
 * Raw SQL row for a proposal from the API (snake_case)
 */
interface ApiProposalRow {
  ...
  execution_eta: number | null;
  signers: string[] | null;
  update_period_end_block: string | null;
  objection_period_end_block: string | null;
  on_timelock_v_1: boolean | null;
...
/**
 * Raw SQL row for votes on a proposal
 */
...
/**
 * Raw SQL row for feedback on a proposal
 */
 ...
 */
function mapProposal(p: ApiProposalRow): Proposal {
  return {
    id: String(p.id),
    title: p.title || '',
...
async function fetchProposals(
  ...
  const json = await response.json();
  const proposals = (json.proposals || []).map((p: ApiProposalRow) => mapProposal(p));
...
async function fetchProposal(id: string): Promise<Proposal & { votes: ProposalVote[]; feedback: ProposalFeedback[] }> {
  ...
  return {
    ...proposal,
    status,
    votes: (p.votes || []).map((v: ApiVoteRow) => ({
      id: v.id,
      voter: v.voter,
    ...
    feedback: (p.feedback || []).map((f: ApiFeedbackRow) => ({
      id: f.id,
      voter: f.msg_sender,
      support: f.support,
