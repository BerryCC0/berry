...
// =============================================================================
// SUBGRAPH 6: ENS DOMAINS
// Indexed from ENSRegistry, BaseRegistrar, ETHRegistrarController, NameWrapper.
// Powers "names for address" lookups without third-party subgraphs.
// =============================================================================

/**
 * Every ENS name we've observed via events.
 * - node is the keccak namehash of the full name
 * - name is the human-readable form when we know it (from Controller or NameWrapper events)
 * - Three ownership fields cover the three places a name can be controlled:
 *     registrant   = BaseRegistrar ERC-721 holder (for .eth 2LDs)
 *     wrappedOwner = NameWrapper ERC-1155 holder (for wrapped names)
 *     owner        = ENSRegistry record owner (everything else, incl. subnames + DNS)
 *   For wrapped names, owner = NameWrapper contract address while wrappedOwner is the user.
 */
export const ensDomains = onchainTable(
  "ens_domains",
  (t) => ({
    ...
    ttl: t.bigint(),
    expiry: t.bigint(),
    isWrapped: t.boolean().notNull().default(false),
    fuses: t.integer(),
    createdAt: t.bigint(),
  ...
  (table) => ({
    ...
    registrantIdx: index().on(table.registrant),
    wrappedOwnerIdx: index().on(table.wrappedOwner),
    parentIdx: index().on(table.parent),
    nameIdx: index().on(table.name),
  }),
);

/**
 * Historical registration events from the ETHRegistrarController.
 * Each row is one .eth registration. Lets us show price history + provenance.
 */
export const ensRegistrations = onchainTable(
  "ens_registrations",
  (t) => ({
    id: t.text().primaryKey(),
    domainNode: t.hex().notNull(),
  ...
  (table) => ({
    registrantIdx: index().on(table.registrant),
    domainIdx: index().on(table.domainNode),
  }),
);
