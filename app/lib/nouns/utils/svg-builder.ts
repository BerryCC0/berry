...
/**
 * Decode the RLE image data into a format that's easier to consume.
 */
...
/**
 * Given an x-coordinate, draw length, and right bound, return the draw length for a single SVG rectangle.
 */
 ...
 */
export const buildSVG = (parts: ImagePart[], paletteColors: string[], bgColor: string): string => {
  const svgWithoutEndTag = parts.reduce((result, part) => {
    ...
    return result;
  }, `<svg width="320" height="320" viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="${bgColor || 'none'}" />`);
